...
JcomentariolineaJ =\/\/[^\n]*
JercharJ = \'(\\\\([\"\'\\\\bfnrt]|u[0-9A-Fa-f]{4})|[^\\\\\'])\'
JidentificadorJ = [a-zA-Z][a-zA-Z0-9_]*
...
"tipode"                {Token NuevoToken = new Token("TkResTIPODE",yytext(),yyline,yycolumn);Funcion.TokenList.add(NuevoToken);System.out.println("Se acepto el Token  <TkResTIPODE> Tk: " + yytext());return new Symbol(sym.TkResTIPODE, yycolumn, yyline, yytext()); }
"true"                  {Token NuevoToken = new Token("TkResTRUE",yytext(),yyline,yycolumn);Funcion.TokenList.add(NuevoToken);System.out.println("Se acepto el Token  <TkResTRUE> Tk: " + yytext());return new Symbol(sym.TkResTRUE, yycolumn, yyline, yytext()); }
"false"                 {Token NuevoToken = new Token("TkResFALSE",yytext(),yyline,yycolumn);Funcion.TokenList.add(NuevoToken);System.out.println("Se acepto el Token  <TkResFALSE> Tk: " + yytext());return new Symbol(sym.TkResFALSE, yycolumn, yyline, yytext()); }
...
{JcomentariomultilineaJ}  {System.out.println("Se acepto el Token  <Comentario Multilinea> Tk: ");}
{JcomentariolineaJ}       {System.out.println("Se acepto el Token  <Comentario de Una linea> Tk: ");}
